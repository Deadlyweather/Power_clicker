...
const Hidden_Stats = [
    ...
    {nimi: "Autoclicker", määrä: 0, cooldown: 1000, aktiivinen: false},
...
const Kauppa = [
    ...
    {id: 7, hinta: 100000, ostot: 0, maksimi: 10},
    ...
    {id: 11, hinta: 1, ostot: 0, maksimi: 1},
    {id: 12, hinta: 100, ostot: 0, maksimi: 100},
...
function BeginCombo() {
    ...
    if (Stats[2].määrä > Stats[2].maksimi) {
        Stats[2].määrä = Stats[2].maksimi;
    }

    Stats[2].kerroin = Stats[2].määrä * Stats[2].kerroinPerCombo;
...
function Click() {
    let Trigger = 0;
    if (Math.random() < 0.01 * Hidden_Stats[3].määrä) Trigger = 1000;
    else Trigger = 1;
...
function Ostatuote(id) {
    let itemIndex = Kauppa.findIndex(X_3 => X_3.id === id);
    if (itemIndex === -1) return;

    let ostettavaa = OstoKerroin;
    let saatavilla = Kauppa[itemIndex].maksimi - Kauppa[itemIndex].ostot;
    if (ostettavaa > saatavilla) ostettavaa = saatavilla;
    ...
    if (Stats[0].määrä >= kokonaishinta && ostettavaa > 0) {
        ...
        switch (id) {
            case 1:
                Stats[0].PerClick = 1 + Kauppa[itemIndex].ostot;
                break;
            case 2:
                Stats[0].kerroin = 1 + Kauppa[itemIndex].ostot;
                break;
            case 3:
                Stats[2].maksimi_elinaika = 1000 + Kauppa[itemIndex].ostot * 10;
                break;
            case 4:
                Stats[2].kerroinPerCombo = 1 + Kauppa[itemIndex].ostot;
                break;
            case 5:
                Hidden_Stats[1].määrä = Kauppa[itemIndex].ostot;
                break;
            case 6:
                Hidden_Stats[0].määrä = 100 * (1 - (0.10 * Kauppa[itemIndex].ostot));
                break;
            case 7:
                Hidden_Stats[2].määrä = Kauppa[itemIndex].ostot;
                Hidden_Stats[2].aktiivinen = Hidden_Stats[2].määrä > 0;
                Autoclicker();
                break;
            case 8:
                Hidden_Stats[3].määrä = Kauppa[itemIndex].ostot;
                Hidden_Stats[3].aktiivinen = Hidden_Stats[3].määrä > 0;
                break;
            case 9:
                Stats[2].PerClick = Kauppa[itemIndex].ostot;
                break;
            case 10:
                if (Kauppa[itemIndex].ostot > 0) Hidden_Stats[4].aktiivinen = true;
                break;
            case 11:
                Stats[3].määrä = Kauppa[itemIndex].ostot + Hidden_Stats[5].määrä * (Hidden_Stats[6].määrä + 1);
                break;
            case 12:
                Hidden_Stats[5].määrä += Kauppa[itemIndex].ostot;
                break;
            case 13:
                Hidden_Stats[6].määrä = Kauppa[itemIndex].ostot;
                break;
            case 14:
                Hidden_Stats[7].määrä = Kauppa[itemIndex].ostot;
                break;
            case 15:
                if (Kauppa[itemIndex].ostot > 0) Hidden_Stats[8].aktiivinen = true;
                break;
            case 16:
                break;
        }
...
function Autoclicker() {
    ...
    // Guard against division by zero and compute spacing between clicks.
    let interval = Hidden_Stats[2].cooldown / Math.max(1, Hidden_Stats[2].määrä);

    if (Hidden_Stats[2].loop) clearInterval(Hidden_Stats[2].loop);
    Hidden_Stats[2].loop = setInterval(() => {
        for (let X_2 = 0; X_2 < Hidden_Stats[2].määrä; X_2++) {
            setTimeout(() => Pressed(), X_2 * interval);
        }
    }, Hidden_Stats[2].cooldown);
}

function Orjat() {
    let baseOrjat = Kauppa[10].ostot;
    if (Hidden_Stats[7].määrä > 0) {
        baseOrjat *= 1 + Hidden_Stats[7].määrä;
    }

    if (Hidden_Stats[5].määrä > 0) {
        Stats[3].määrä = baseOrjat + Hidden_Stats[5].määrä * (Hidden_Stats[6].määrä + 1);
    } else {
        Stats[3].määrä = baseOrjat * (Hidden_Stats[6].määrä + 1);
    }

    Stats[0].määrä += Stats[3].määrä * Stats[2].kerroin * Hidden_Stats[5].määrä;
